...
backtest<-function(u,n,dir=1,w=91){
  u_tr<-universe2matrix(u)
  u_sig<-dir*tret2sig(u_tr,w=w)
  ...
  data.table(
    ...
    pair_sharpe=list(apply(u_perf,2,function(x)round(sqrt(356)*mean(x)/sd(x),digits=2))),
    strat=list(strat),
    sharpe=round(sqrt(356)*mean(strat)/sd(strat),digits=2),
    vol=round(100*16*sd(strat),digits=2),
    perf=round(100*356*mean(strat),digits=2)
  )
}
...
# backtest
results=list(
  trend182_results=do.call(rbind,mapply(backtest,u=subsets,n=names(subsets),MoreArgs=list(dir=1,w=182),SIMPLIFY=FALSE)),
  trend91_results=do.call(rbind,mapply(backtest,u=subsets,n=names(subsets),MoreArgs=list(dir=1,w=91),SIMPLIFY=FALSE)),
  trend30_results=do.call(rbind,mapply(backtest,u=subsets,n=names(subsets),MoreArgs=list(dir=1,w=30),SIMPLIFY=FALSE)),
  rev182_results=do.call(rbind,mapply(backtest,u=subsets,n=names(subsets),MoreArgs=list(dir=-1,w=182),SIMPLIFY=FALSE)),
  rev91_results=do.call(rbind,mapply(backtest,u=subsets,n=names(subsets),MoreArgs=list(dir=-1,w=91),SIMPLIFY=FALSE)),
  rev30_results=do.call(rbind,mapply(backtest,u=subsets,n=names(subsets),MoreArgs=list(dir=-1,w=30),SIMPLIFY=FALSE)),
  rev14_results=do.call(rbind,mapply(backtest,u=subsets,n=names(subsets),MoreArgs=list(dir=-1,w=14),SIMPLIFY=FALSE))
)

results2df<-function(x)data.table(
    name=x$name,
    strat=x$type,
    window=x$window,
    pairs=structure(mapply(length,x$pairs),hdr=tbl(c("Pair","count"),align="@{}l")),
    sharpe=structure(x$sharpe,hdr="Sharpe",format=quote(n_fmt(this))),
    min_sharpe=structure(mapply(min,x$pair_sharpe),hdr=tbl(c("Min","sharpe"),align="@{}l"),format=quote(n_fmt(this))),
    max_sharpe=structure(mapply(max,x$pair_sharpe),hdr=tbl(c("Max","sharpe"),align="@{}l"),format=quote(n_fmt(this))),
    vol=structure(x$vol,hdr="Volatility",format=quote(n_fmt(this))),
    perf=structure(x$perf,hdr="Performance",format=quote(n_fmt(this)))
  )

all_df<-do.call(rbind,mapply(results2df,results,SIMPLIFY = FALSE))[order(name)]

results2plot<-function(x)data.table(
...
gg_trend<-results2plot(results$trend91_results) %>% 
  ggplot() +
  geom_line(aes(x=date,y=perf*100,col=group,group=group)) +
  ggtitle(paste0("91-day Trend"))

gg_rev<-results2plot(results$rev14_results) %>% 
  ggplot() +
  geom_line(aes(x=date,y=perf*100,col=group,group=group))+
  ggtitle(paste0("14-day Reversion"))


@

We attempt to verify results presented in the document by testing a very simple
time-series trend strategy on the proposed universe as well as a wider one.
The signal is the sign of rolling returns over a selection of windows. 
Results seem to agree with the results presented in the document.
