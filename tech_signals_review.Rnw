...
The document begins by introducing the singals used by the proposed strategy. As per the document, these signals are considered 
to be ``robust representations of factor risk premia``. These risk premia are: {\bf carry}, {\bf value}, {\bf momentum} and 
{\bf volatility}.The specific mapping is not clearly and explicitly presented in the introduction, but it is fairly 
straight-forward \footnote{Ang, Andrew. A systematic Approach to Factor Investing. Oxford University Press 2014}, with the
exception of ``yield momentum''.
...
\hline
\rowcolor{gray!20}
Risk factors associated to risk premia and asset classes in ANG, 2014 \\
\Sexpr{web_png(
...
rise during market-wide risk-off periods, even if they are uncorrelated during other periods. These ``unusual'' risk factors
are sometimes called ``market anomalies'' as opposed to  ``risk-premia'' \footnote{For example see the introduction in Tobias 
J. Moskowitz, Yao Hua Ooi, Lasse Heje Pedersen. Time series momentum. Journal of Financial Economics 104(2012) 228-250}. It is
worth noting that there are many more ``anomalies'' than there are pure ``risk premia''.

\vskip 5mm
\begin{itemize}
\item \href{https://s3.amazonaws.com/papers-documentation/Books+-+Market+Factors/(Financial+Management+Association+Survey+and+Synthesis+Series)+Andrew+Ang-Asset+Management_+A+Systematic+Approach+to+Factor+Investing-Oxford+University+Press+(2014)+(1).pdf}{Ang, Andrew. A systematic Approach to Factor 
Investing. Oxford University Press 2014}
\end{itemize}
...
Summary of technical signal investigation:

\vskip 5mm

\begin{enumerate}
\item Although momentum and reversion are assigned to different risk factors, their implementation is identical, up to a sign. 
\item Both price and momentum reversion rely on a trend signal, but they trade ``opposite'' positions. 
\item Price reversion sells the asset after positive performance and buys after negative performannce. 
\item Price momentum buys after gains and sells after losses.
\item We verify that these signals are derived from a longitudinal, time-series trend indicator.
\item We examine the parameter space and asses potential for over-fitting inherent in parameter choices made.
\item A trend strategy has 2 important parameters: The {\bf investment universe} and the {\bf trend estimation window}.
\item To evaluate how the signal behaves over its parameters space, we need to run the strategy multiple times, with different parameters.
\item We use a very, very simple trend model as a proxy for the ``real'' strategy. The fit is suprisingly good, we expect conclusions
relating to this proxy to carry over well to the actual model.
\end{enumerate}


\subsubsection{Actual QCR price momentum signal vs proxy price momentum signal}
<<,cache=FALSE, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, results="hide">>=

sharpe<-function(strat)sqrt(356)*mean(strat)/sd(strat)

# make matrix of signals: sign of rolling total returns
tret2sig <- function(x,w=91,thresh=0.05){
...
gg_cv<- cv %>%
  ...
  ggtitle(paste0("QCR-Benchmark signal correlation vs Benchmark window plots"))
...
\begin{enumerate}
\item After some investigation we identified a simple signal matching observed signal values: location in
rolling price range. This signal appears to be a better match than simple total returns, so we used it.
\item As per the document, our universe is all currency pairs derived from the g10 set of currencies.
\item We use a momentum strategy with the trend signal applied to pairs derived from the g3 universe subset
\item We use a reversion strategy with the trend signal applied to pairs derived from the g10xg3 universe subset
\item We select a window size that maximizes correlation between proxy signals and actual signals
\item Achieved correlation of daily returns is high: \Sexpr{round(100*cor(qcr_strat,qcrb_strat),digits=1)}. 
We believe this suggests that our benchmark captures a large part of 
actual signal behaviour
\item We can now use our proxy to understand the parameter space of the price momentum and price reversion signals
\end{enumerate}
...
we apply the trend model to currency universes of increasing size. we then examine the 
...
\begin{itemize}
\item adding further currency offers no diversification benefit
\item instead, additional currencies sytematically reduce performance
\item this is not one would expect if the trend signal worked across the universe
\item it seems that the price momentum signal is limited to the g3 subsets because this is the subset
on which the signal performs best
\item It is therefore important to justify the reasons for limiting the price momentum signal to the 
g3 subset
\item However, the subset sellection appears to be capturing some property of the time-series. g3
appears to work consistently over quite a long period
\end{itemize}
...
parameter selection is compared others.
