...
What we actually see is that growing the universe beyond g3 is systematically harmful
...
\begin{itemize}
\item Backtest results suggest that the technical signals capture a persistent source of alpha
\item Returns are not due to risk premium collected through exposure to risk factors.
\item Pure risk factor exposure does not appear to be profitable in the selected universe
\item Almost all returns are coming from a specific view on structural differences in parts
of the FX market
\item The excess return due to this view is makes this strategy more valuable than a simple 
beta product. On the other hand, it might prevent marketing it as such
