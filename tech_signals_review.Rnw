...
\begin{enumerate}
\item Analysis of the indicator used by the reversion and the momentum strategies. These appear to be driven by 
the same indicator
\item Review of proposed currency split and arguments motivating presented choises
...
\item Remaining pairs are not used
\item We select a window size that maximizes correlation between proxy signals and actual signals
\item Achieved correlation of daily returns to the actual proposed strategy is high: {\bf 
\Sexpr{round(100*cor(qcr_strat,qcrb_strat),digits=1)}}. It is very likely that actual signals
are quite similar to the ones used by the proxy. The benchmark captures a large part of actual 
signal behaviour, conclusions should transfer well
\item momentum and reversion strategies use the same type of signal
\end{enumerate}
...
\item Almost all returns are coming from two specific views relating to features of the FX market
\item Firstly, that the g3 subset trends while the g10xg3 subset mean-reverts.
\item Secondly, that price reversions are faster and require a shorter signal window than
momentum strategies
\item These views have worked and this excess performance makes the strategy more attractice 
than a simple beta product. On the other hand, this same reason might prevent marketing
the strategy as being purely driven by risk premia
