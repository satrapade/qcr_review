...
# mean(diag(cor(tail(qcr_stvf,-ss),tail(-tret2sig_rngloc(qcr_tr,w=250),-ss))))
# mean(diag(cor(tail(qcr_stvf,-ss),tail(-tret2sig_tret(qcr_tr,w=60),-ss))))
# mean(diag(cor(tail(qcr_stvf,-ss),tail(-tret2sig_macross(qcr_tr,wf=90,ws=250),-ss))))
# 
# cs1<- tail(-tret2sig_rngloc(qcr_tr,w=250),-ss) +
#   5*tail(-tret2sig_tret(qcr_tr,w=60),-ss) +
#   0.5*tail(-tret2sig_macross(qcr_tr,wf=90,ws=250),-ss)
# 
# mean(diag(cor(tail(qcr_stvf,-ss),cs1)))
# 
# x<-c(rep(0,59),roll_mean(qcr_stvf[-(1:ss),1],60))
# x1<-c(rep(0,179),roll_mean(qcr_stvf[-(1:ss),1],180))
# y<-c(rep(0,179),200*roll_mean(qcr_tr[-(1:ss),1],180))
# plot(-(x-x1),col=rgb(0,1,0,0.25),pch=19,ylim=c(-0.5,0.5))
# lines(y,col="red")
...
\\
\hline
{\bf pm} & g3 & price momentum & technical & momentum & high & \Sexpr{round(sharpe(tail(signal_df$pm,-ss)),digits=2)} \\
{\bf pr} & g10xg3 & price reversion & technical & volatility & medium & \Sexpr{round(sharpe(tail(signal_df$pr,-ss)),digits=2)} \\
{\bf cm} & g10 & yield momentum &  fundamental & {\bf hard to tell} & low & \Sexpr{round(sharpe(tail(signal_df$cm,-ss)),digits=2)}\\
{\bf cl} & g10 & risk-adjusted yield & fundamental & carry & high & \Sexpr{round(sharpe(tail(signal_df$cl,-ss)),digits=2)}\\
{\bf lv} & g10 & long-term fair value & fundamental & value & high & \Sexpr{round(sharpe(tail(signal_df$lv,-ss)),digits=2)}\\
{\bf sv} & g10 & short-term fair value & fundamental & value & low & \Sexpr{round(sharpe(tail(signal_df$sv,-ss)),digits=2)}\\
\end{tabular}
\end{center}
...
x1<-t(apply(apply(qcr_tr,2,roll_sumr,n=30,fill=0),1,rank))/ncol(qcr_tr)-0.5

y1<-apply(qcr_tr,2,function(x){
  x1<-roll_sdr(x,30,fill=0)
  x2<-roll_meanr(x,30,fill=0)
  ifelse(x1>0,x2/x1,0)
})

y2<-t(apply(y1,1,rank))/ncol(y1)-0.5

mean(diag(cor(
  qcr_stvf[all_signals_live,],
  (-1)*proxy[all_signals_live,]+
  (+1)*qcr_carry_level[all_signals_live,]+
  (+1)*qcr_carry_momentum[all_signals_live,]+
  (-0.5)*x1[all_signals_live,]+
  (-0.5)*y1[all_signals_live,]
)))

pnl0<-(-(head(proxy,-1)*tail(qcr_tr,-1))[tail(all_signals_live,-1),])%>%
  {.%*%diag(0.05/apply(.,2,sd))}

pnl1<-((head(qcr_stvf,-1)*tail(qcr_tr,-1))[tail(all_signals_live,-1),])%>%
  {.%*%diag(0.05/apply(.,2,sd))}

pnl2<-((head(qcr_stvf*(sign(qcr_stvf)==sign(-proxy)),-1)*tail(qcr_tr,-1))[tail(all_signals_live,-1),])%>%
  {.%*%diag(0.05/apply(.,2,sd))}

pnl3<-((head(qcr_stvf*(sign(qcr_stvf)!=sign(-proxy)),-1)*tail(qcr_tr,-1))[tail(all_signals_live,-1),])%>%
      {.%*%diag(0.05/apply(.,2,sd))}

pnl4<-(-(head(proxy*(sign(qcr_stvf)!=sign(-proxy)),-1)*tail(qcr_tr,-1))[tail(all_signals_live,-1),])%>%
      {.%*%diag(0.05/apply(.,2,sd))}

pnl5<-(-(head(proxy*(sign(qcr_stvf)==sign(-proxy)),-1)*tail(qcr_tr,-1))[tail(all_signals_live,-1),])%>%
      {.%*%diag(0.05/apply(.,2,sd))}

pnl6<-(-(head(proxy*(sign(qcr_stvf)==0),-1)*tail(qcr_tr,-1))[tail(all_signals_live,-1),])%>%
      {.%*%diag(0.05/apply(.,2,sd))}

#plot(cumsum(rowSums(pnl4+pnl5+pnl6)/3))
#plot(cumsum(rowSums(pnl0)))

@
\newpage
\subsection{Signal plots: columns are pairs ordered by average carry. Signal, proxy correlations}
