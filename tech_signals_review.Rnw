...
results=list(
  ...
  rev45_results=do.call(rbind,mapply(backtest,u=subsets,n=names(subsets),MoreArgs=list(dir=-1,w=45),SIMPLIFY=FALSE)),
...
gg_rev<-results2plot(results$rev45_results) %>% 
  ggplot() +
  geom_line(aes(x=date,y=perf*100,col=group,group=group))+
