...
tret2sig <- function(x,w=91){
  ...
  rng<-structure(apply(x,2,function(p){
    ...
    rng<-2*((tail(px,-(w-1))-pxmin)/(pxmax-pxmin)-0.5)
    c(rep(0,w-1),rng)
  }),dimnames=dimnames(x))
  rng*(abs(rng)>0.25)
}
...
ws<-seq(from=10,to=365,length.out = 25)

cv<-local({ 
  strat<-cbind(s_long=qcr_strat_long,s_short=qcr_strat_short)
  cf<-function(w){
    cm<-cor(qcr_msig,tret2sig(qcr_rp,w))
...
\begin{enumerate}
\item We use the simplest possible trend signal: sign of rolling total returns.
