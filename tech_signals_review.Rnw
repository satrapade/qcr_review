...
@
\newpage
\subsection{Signal plots: columns are pairs ordered by average carry. Correlation: signals and proxy}
...
@
\newpage
\subsection{Cross-correlation functions: signals vs rolling returns }
