...
backtest<-function(n,u,dir=1,w=91){ # name, universe, direction, window
...
backtest_subset<-function(n,u,dir,w,source_backtest){
  u_pairs<-universe2pairs(u)
  u_perf<-source_backtest$pair_perf[[1]][,u_pairs]
  strat<-rowMeans(u_perf)
  data.table(
    name=n,
    type=ifelse(dir>0,"trend","reversion"),
...
batch_all_91trend_result<-backtest("U",colnames(ccy_tr),91,1)

batch_trend_3subsets_result<-c(
  backtest_subset,
  batch_trend_3subsets,
  MoreArgs=list(source_backtest=batch_all_91trend_result),
  SIMPLIFY=FALSE
) %>%
{do.call(mapply,.)} %>%
{do.call(rbind,.)}
...
batch_trend_7subsets_result<-do.call(rbind,do.call(mapply,c(backtest,batch_trend_7subsets,SIMPLIFY=FALSE)))




gg_trend<-results2plot(batch_result[type=="trend" & window==91]) %>% 
  ggplot() +
