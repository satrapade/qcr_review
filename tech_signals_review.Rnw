...
@
\newpage
\subsection{Signal tiles: columns are pairs ordered by average carry. Correlation: signals and proxy}
...
\newpage
\subsection{Combined signals: signal tile, cummulative sign plot}
<<,cache=FALSE, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, results="hide">>=
combined<-( 
